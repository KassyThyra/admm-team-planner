...
function App() {
  ...
  async function replaceTaskDependencies(taskId, dependencyIds = []) {
    await supabase.from("task_dependencies").delete().eq("task_id", taskId);
    const cleaned = Array.from(new Set((dependencyIds || []).filter(Boolean).filter(id => id !== taskId)));
    if (cleaned.length) {
  ...
  return (
    <main className="page">
      ...
      {tab === "me" && <MyArea profile={profile} activeSprint={activeSprint} tasks={myTasks} schedule={mySchedule} meetings={meetings} form={scheduleForm} setForm={setScheduleForm} addSchedule={addSchedule} deleteSchedule={deleteSchedule} nameOf={nameOf} assigneesOf={assigneesOf} commentsOf={commentsOf} filesOf={filesOf} moveTask={moveTask} patchTask={patchTask} deleteTask={deleteTask} addComment={addComment} uploadTaskFile={uploadTaskFile} deleteTaskFile={deleteTaskFile} replaceTaskDependencies={replaceTaskDependencies} />}
...
function normalizedOrderStatus(status) {
  if (status === "Shipped") return "Ordered";
  return orderStatus.includes(status) ? status : "Needed";
}
function orderStatusRank(status) {
  return ({Needed: 0, Ordered: 1, Arrived: 2})[normalizedOrderStatus(status)] ?? 0;
}
function sortedOrdersList(orders) {
  return [...orders].sort((a,b) => orderStatusRank(a.status) - orderStatusRank(b.status) || String(a.name || "").localeCompare(String(b.name || "")));
}
function orderStatusClass(status) {
  return `orderRow ${normalizedOrderStatus(status).toLowerCase()}`;
}
...
function MyArea({profile,activeSprint,tasks,schedule,meetings,form,setForm,addSchedule,deleteSchedule,nameOf,assigneesOf,commentsOf,filesOf,moveTask,patchTask,deleteTask,addComment,uploadTaskFile,deleteTaskFile,replaceTaskDependencies}) {
  return <section className="grid two">
    <div className="stack">
      ...
      <Card title="Costs">
        ...
        {shippingItems.length > 0 && <div className="shippingCostList">
          {shippingItems.map(item => <div className="fileRow" key={item.id}>
            ...
            <button className="miniDelete" type="button" onClick={()=>deleteShippingCost(item.id)}><Trash2 size={14}/></button>
          </div>)}
        </div>}
...
function EditableOrder({order,profiles,patchOrder,deleteOrder,nameOf,files,uploadGenericFile,deleteGenericFile}) {
  ...
  const [draft,setDraft]=useState({...order, status: normalizedOrderStatus(order.status)});
  async function save() {
  ...
  const itemTotal = parsePriceValue(order.price) * quantity;
  return <div className="taskCard compactTask">
    <div className="row clickable" onClick={()=>setExpanded(!expanded)}>
      <strong>{order.name || order.item || "Ordering"}</strong>
      <span className={`badge orderBadge ${status.toLowerCase()}`}>{status}</span>
    </div>
    {expanded && <>
      {!editing ? <>
        <p>{order.description}</p>
        <p className="muted">{order.shop || "No shop"} · Quantity {order.quantity || "1"} · Unit price {order.price || "-"} · Total {formatEuro(itemTotal)} · Owner: {nameOf(order.owner_id)} · Location: {order.location || "-"}</p>
        ...
        <select value={normalizedOrderStatus(draft.status)} onChange={e=>setDraft({...draft,status:e.target.value})}>{orderStatus.map(s=><option key={s}>{s}</option>)}</select>
      </div>}
...
function EditableSprint({sprint,patchSprint,deleteSprint,files,uploadGenericFile,deleteGenericFile}) {
  ...
  return <div className="taskCard compactTask">
    <div className="row clickable" onClick={()=>setExpanded(!expanded)}><strong>{sprint.name}</strong><span className="badge">{sprint.status}</span></div>
    {expanded && <>
      {!editing ? <>
...
function EditableMeeting({meeting,patchMeeting,deleteMeeting,files,uploadGenericFile,deleteGenericFile}) {
  ...
  return <div className="taskCard compactTask">
    <div className="row clickable" onClick={()=>setExpanded(!expanded)}>
      ...
      <span className="badge">{meeting.meeting_type}</span>
    </div>
    {expanded && <>
      {!editing ? <>
...
function EditableInfoItem({item,patchInfoItem,deleteInfoItem,nameOf,files,uploadGenericFile,deleteGenericFile}) {
  ...
  return <div className="taskCard compactTask">
    <div className="row clickable" onClick={()=>setExpanded(!expanded)}><strong>{item.title}</strong><span className="badge">Info</span></div>
    {expanded && <>
      {!editing ? <>
...
function TaskCard({task,profile,nameOf,assigneesOf,depsOf,comments,files,moveTask,patchTask,deleteTask,addComment,uploadTaskFile,deleteTaskFile,removeTaskFromSprint,sprints=[],allTasks=[],replaceTaskDependencies,highlightCurrentSprint=false}) {
  const [open,setOpen]=useState(false);
  ...
  const taskCardClassName = task.deadline&&new Date(task.deadline)<startOfToday()&&task.status!=="Done" ? "taskCard overdue" : "taskCard";
  const currentSprintStyle = highlightCurrentSprint ? { borderLeft: "6px solid #dc2626", background: "#fef2f2" } : undefined;
  return <div className={taskCardClassName} style={currentSprintStyle}>
