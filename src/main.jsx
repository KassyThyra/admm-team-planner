...
function TaskCard({task,profile,nameOf,assigneesOf,depsOf,comments,files,moveTask,patchTask,deleteTask,addComment,uploadTaskFile,removeTaskFromSprint}) {
  const [comment,setComment]=useState(""); const taskAssignees=assigneesOf(task.id); const canDelete=profile?.is_pm||task.owner_id===profile?.id; const deps=depsOf(task.id);
